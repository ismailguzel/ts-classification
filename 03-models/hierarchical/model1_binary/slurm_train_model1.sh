...
# Job bilgisi
echo "=================================================="
echo "TRUBA Model 1 Training - Binary Classification"
echo "=================================================="
...
if [ $TRAINING_EXIT_CODE -eq 0 ]; then
    ...
    echo "Saved Models:"
    if [ -d "models" ]; then
        ls -lh models/model1_*.pkl 2>/dev/null || echo "  No model files found"
    fi
    ...
    if [ -d "results" ]; then
        echo ""
        echo "Results:"
        ls -lh results/model1_*.txt 2>/dev/null || echo "  No result files found"
    fi
    ...
    echo "Check logs for details: logs/train_model1_${SLURM_JOB_ID}.err"
