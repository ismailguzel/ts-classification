...
- feature/label loading (`load_features_and_labels`)
- leakage-safe splits (`remove_series_id_leakage`, `split_train_test`)
- evaluation + reporting (`ModelEvaluator`)

Supports AutoGluon for automated binary classification.
"""
...
def train_autogluon(
    *,
    ...
    """Train an AutoGluon TabularPredictor using shared utils and evaluators."""
...
def main():
    parser = argparse.ArgumentParser(description="AutoTrain Model 1 (binary) with AutoGluon")
    ...
    print("=" * 80)
    print("MODEL 1 AUTOTRAIN - Binary Classification (AutoGluon)")
    print("=" * 80)
    ...
    X_df, y, _ = load_features_and_labels(features_path, target="binary", verbose=True)
    ...
    meta_path = result.save_path / "model1_autotrain_metadata.pkl"
    ...
    print(f"   ├── predictions_AutoGluon_Binary.csv     # Test predictions with probabilities")
    print(f"   ├── misclassified_AutoGluon_Binary.csv   # Misclassified samples")
    print(f"   ├── leaderboard_test.csv                 # Test performance leaderboard")
    print(f"   ├── leaderboard_train.csv                # Train performance leaderboard")
    print(f"   ├── model1_autotrain_metadata.pkl        # Training metadata")
    print(f"   └── metrics/")
    print(f"       └── AutoGluon_Binary_metrics.json    # Detailed metrics")
    print("=" * 80)
