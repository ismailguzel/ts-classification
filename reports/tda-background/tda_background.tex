...
The five Carlsson coordinates are:

\begin{align}
  f_1(D) &= \sum_i p_i^2 \label{eq:carl_f1} \\[4pt]
  f_2(D) &= \sum_i (d_{\max} - d_i)^2 \cdot p_i^2 \label{eq:carl_f2} \\[4pt]
  f_3(D) &= \sum_i b_i \cdot p_i^2 \label{eq:carl_f3} \\[4pt]
  f_4(D) &= \sum_i (d_{\max} - d_i) \cdot p_i^4 \label{eq:carl_f4} \\[4pt]
  f_5(D) &= \sum_i b_i^2 \cdot p_i^4 \label{eq:carl_f5}
\end{align}

where $d_{\max} = \max_i d_i$. The quadratic and quartic weights in $p_i$
down-weight low-persistence (noise) points and emphasise high-persistence
(signal) features. Coordinates $f_1$ and $f_2$ measure the total squared
persistence energy and how it is distributed near the death axis, respectively.
Coordinate $f_3$ captures the birth-weighted persistence — a proxy for how
early in the filtration the most persistent features appear.
...
We extract two scalar features from the first two landscape functions:
\begin{equation}
  \ell_1 = \|\lambda_1\|_1 = \int |\lambda_1(t)|\, dt, \qquad
  \ell_2 = \|\lambda_2\|_1 = \int |\lambda_2(t)|\, dt.
  \label{eq:landscapes}
\end{equation}

These $L^1$ norms measure the ``area'' under the dominant and secondary landscape
functions. $\ell_1$ is dominated by the most persistent feature; $\ell_2$ reflects
the second-tier structure. Together they capture the dominance hierarchy of
...
\midrule
1 & \texttt{carl\_f1} & \eqref{eq:carl_f1} & Total squared persistence energy \\
2 & \texttt{carl\_f2} & \eqref{eq:carl_f2} & Persistence weighted by proximity to death axis \\
3 & \texttt{carl\_f3} & \eqref{eq:carl_f3} & Birth-weighted persistence \\
4 & \texttt{carl\_f4} & \eqref{eq:carl_f4} & Death-proximity $\times$ quartic persistence \\
5 & \texttt{carl\_f5} & \eqref{eq:carl_f5} & Birth$^2$ $\times$ quartic persistence \\
6 & \texttt{entropy}  & \eqref{eq:pers_entropy} & Complexity / spread of persistence values \\
7 & \texttt{landscape\_l1} & \eqref{eq:landscapes} & $L^1$ norm of the first landscape function \\
8 & \texttt{landscape\_l2} & \eqref{eq:landscapes} & $L^1$ norm of the second landscape function \\
\bottomrule
\end{tabular}
\end{table}
...
and \texttt{feature} $\in \{\texttt{carl\_f1}, \ldots, \texttt{carl\_f5},\,
\texttt{entropy},\, \texttt{landscape\_l1},\, \texttt{landscape\_l2}\}$.
...
\medskip
\noindent
\textbf{Selection result.} Of the 24 raw topological features, 18 are retained by
MI selection on the 10-class training split. The 6 dropped features
(\texttt{carl\_f5} from all three diagrams, and \texttt{landscape\_l2} from
\texttt{sup\_H0}) carry near-zero MI with the class label, indicating they encode
noise rather than class-discriminating structure.
...
accuracy from just 18 features — a compression ratio of 5.6$\times$ relative to
the 100-feature TSFresh pipeline, with only a 8.5 percentage point accuracy
...
Topological features thus deliver approximately \textbf{5$\times$ better
accuracy-per-feature efficiency} than the statistical baseline.
...
\begin{itemize}
  \item \textbf{Trend shift:} A slope change creates a distinctive feature in the
    superlevel filtration --- two connected components that merge at the apex of
    the change. The superlevel persistence diagram captures this as a single
    high-persistence point whose death value corresponds to the change-point
    amplitude. TSFresh chunk linear trend features miss this when the change-point
...
This Carlsson coordinate measures the birth-weighted persistence of sublevel
connected components (Equation~\ref{eq:carl_f3}):
\[
  f_3(D) = \sum_i b_i \cdot (d_i - b_i)^2,
\]
a weighted summary of when (at which threshold level) the most persistent
components first appear. This feature is highly discriminative because different
time series types have fundamentally different sublevel connectivity patterns:
trends have an asymmetric global minimum; anomalies introduce secondary persistent
minima; stationary processes have many short-lived low-persistence components.
...
\begin{enumerate}
  \item Achieve 86.5\% classification accuracy on a 10-class synthetic benchmark
    with only 18 features (5$\times$ feature efficiency of TSFresh);
